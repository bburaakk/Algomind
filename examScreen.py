...
class TestScreen(BaseScreen):
    ...
    def _load_questions_thread(self):
        ...
        # Bulunduğumuz dosyanın yolu: C:\Users\User\PycharmProjects\Algomind\algomind\screens
                 base_path = os.path.dirname(os.path.abspath(__file__))

        # Sadece bir dizin yukarı çıkarak `algomind` klasörüne ulaş
        # algomind/screens -> algomind
                 algomind_folder_path = os.path.dirname(base_path)

        # `algomind` klasöründen sonra `assets` ve `image_folder` yolunu ekle
                 image_path = os.path.join(algomind_folder_path, "assets", image_folder, random.choice(data_dict[correct_item]))

        # Düzeltmenin işe yaradığından emin olmak için yolu yazdırın
    ...
    # TestScreen.py dosyasında

    import os
    # Diğer importlar ...

    # TestScreen sınıfının tanımı ...
    ...
    def display_question(self):
        ...
        if self.test_type in ['animal', 'food', 'object', 'fruit_vegetable']:
            ...
            if os.path.exists(image_path_from_question):
                ...
                print(f"HATA: Resim dosyası bulunamadı: {image_path_from_question}")
            ...
            # Renk testleri için resim yolu oluşturma
            base_path = os.path.dirname(os.path.abspath(__file__))
            algomind_folder_path = os.path.dirname(base_path)
            image_path_color = os.path.join(algomind_folder_path, "assets", COLOR_IMAGE_MAP.get(color_name))

            if os.path.exists(image_path_color):
                image = Image(source=image_path_color, allow_stretch=True, keep_ratio=True)
                self.ids.image_area.add_widget(image)
            else:
                print(f"HATA: Renk görseli bulunamadı: {image_path_color}")
                placeholder = MDLabel(text="Görsel bulunamadı.", halign='center')
                self.ids.image_area.add_widget(placeholder)
        ...
        # Seçenek butonlarını oluştur ve `options_area`'ya ekle
