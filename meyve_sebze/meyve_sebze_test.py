...
# Meyve ve sebze listesi ve dosya adları (images klasöründeki dosyalar)
MEYVE_SEBZE_DATA = {
    "armut": ["armut.png"],
    "domates": ["domates.png"],
    "elma": ["elma.png"],
    ...
    "kiraz": ["kiraz.png"],
    "limon": ["limon.png"],
    "muz": ["muz.png"],  # mısır yerine muz olarak düzeltildi
    "patates": ["patates.png"],
    "portakal": ["portakal.png"],
    "salatalık": ["salatalık.png"],
    "çilek": ["çilek.png"],
    "şeftali": ["şeftali.png"],
    "üzüm": ["üzüm.png"],
}
...
class TestSelectionScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Meyve Sebze Tanıma Testi Butonu
        self.fruit_vegetable_test_button = Button(
            text='Meyve Sebze Tanıma Testi',
            size_hint_y=None,
            height=dp(60),
            font_size='22sp',
            background_normal='',
        ...
        with self.fruit_vegetable_test_button.canvas.before:
            self.fruit_vegetable_test_button.btn_color = Color(*self.fruit_vegetable_test_button.background_color)
            self.fruit_vegetable_test_button.btn_rect = RoundedRectangle(pos=self.fruit_vegetable_test_button.pos,
                                                                         size=self.fruit_vegetable_test_button.size, radius=[dp(15)] * 4)
            self.fruit_vegetable_test_button.bind(pos=update_button_canvas, size=update_button_canvas)
        self.fruit_vegetable_test_button.bind(on_press=self.start_fruit_vegetable_test)
        self.main_layout.add_widget(self.fruit_vegetable_test_button)
    ...
    def start_fruit_vegetable_test(self, instance):
...
class TestQuestionsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Resim (Meyve/Sebze)
        self.fruit_vegetable_image = Image(
            source='images/elma.png',  # Varsayılan resim
            size_hint=(None, None),
            size=(dp(150), dp(150)),
            pos_hint={'center_x': 0.5}
        )
        self.main_layout.add_widget(self.fruit_vegetable_image)
        ...
        self.time_elapsed = 0
        self.test_questions_data = []  # Meyve ve sebze sorularını burada saklayacağız
    ...
    def start_test(self, student_name):
        ...
        # Meyve ve sebze sorularını hazırla
        threading.Thread(target=self._prepare_fruit_vegetable_questions_thread).start()

    def _prepare_fruit_vegetable_questions_thread(self):
        """Meyve ve sebze sorularını hazırlayan thread."""
        try:
            questions = self.prepare_fruit_vegetable_questions()
            if questions:
    ...
    def prepare_fruit_vegetable_questions(self):
        """Images klasöründeki görselleri kullanarak meyve ve sebze tanıma soruları hazırlar."""
        questions = []
        available_items = list(MEYVE_SEBZE_DATA.keys())

        # Mevcut dosyaları kontrol et
        if not os.path.exists('../hayvan_meyveler/images'):
            print("Images klasörü bulunamadı!")
            return None

        # Meyve ve sebze listesini karıştır
        random.shuffle(available_items)

        for correct_item in available_items[:10]:  # Her meyve/sebzeden 1 tane al, toplam 10
            item_images = MEYVE_SEBZE_DATA[correct_item]
            selected_image = random.choice(item_images)
            image_path = f'../hayvan_meyveler/images/{selected_image}'
            ...
            # Yanlış seçenek için farklı bir meyve/sebze seç
    ...
    def load_question(self):
        if self.current_question_index < self.total_questions:
            ...
            # Resmi yükle
            try:
                self.fruit_vegetable_image.source = question_data["image"]
            except Exception as e:
                print(f"Resim yükleme hatası: {e}")
                self.fruit_vegetable_image.source = 'images/default.jpg'  # Varsayılan resim
...
class ReportScreen(Screen):
    ...
    def _fetch_gemini_comment_thread(self, report_data):
        ...
        Bu meyve ve sebze tanıma testi sonuçlarına göre öğrencinin:
        - Görsel algı ve tanıma becerilerini değerlendir
        - Meyve ve sebzeler hakkındaki genel bilgi düzeyini yorumla
        - Sağlıklı beslenme farkındalığını analiz et
        - Konsantrasyon ve dikkat süresini değerlendir
