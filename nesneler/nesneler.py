...
# Nesne listesi ve dosya adları (objects_images klasöründeki dosyalar) - Güncellendi
OBJECT_DATA = {
    "araba": ["araba.png"],
    "balon": ["balon.png"],
    "ev": ["ev.png"],
    ...
    "kapı": ["kapı.png"],
    "kitap": ["kitap.png"],
    "kitaplık": ["kitaplık.png"],
    "masa": ["masa.png"],
    "pencere": ["pencere.png"],
    "sandalye": ["sandalye.png"],
    "televizyon": ["televizyon.png"],
    "yapboz": ["yapboz.png"],
    "yatak": ["yatak.png"],
}
...
class TestSelectionScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Nesne Tanıma Testi Butonu - Güncellendi
        self.object_test_button = Button(
            text='Nesne Tanıma Testi',
            size_hint_y=None,
            height=dp(60),
            font_size='22sp',
            background_normal='',
        ...
        with self.object_test_button.canvas.before:
            self.object_test_button.btn_color = Color(*self.object_test_button.background_color)
            self.object_test_button.btn_rect = RoundedRectangle(pos=self.object_test_button.pos,
                                                                size=self.object_test_button.size, radius=[dp(15)] * 4)
            self.object_test_button.bind(pos=update_button_canvas, size=update_button_canvas)
        self.object_test_button.bind(on_press=self.start_object_test)
        self.main_layout.add_widget(self.object_test_button)

        # Diğer test butonları eklenebilir
        test_names = ["Matematik Testi", "Okuma Testi", "Yazma Testi", "Mantık Testi", "Hafıza Testi"]
    ...
    def start_object_test(self, instance): # Fonksiyon adı güncellendi
...
class TestQuestionsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Resim (Nesne) - Güncellendi
        self.object_image = Image(
            source='objects_images/süt.png',  # Varsayılan resim
            size_hint=(None, None),
            size=(dp(150), dp(150)),
            pos_hint={'center_x': 0.5}
        )
        self.main_layout.add_widget(self.object_image)
        ...
        self.time_elapsed = 0
        self.test_questions_data = []  # Nesne sorularını burada saklayacağız - Güncellendi
    ...
    def start_test(self, student_name):
        ...
        # Nesne sorularını hazırla - Güncellendi
        threading.Thread(target=self._prepare_object_questions_thread).start()

    def _prepare_object_questions_thread(self): # Fonksiyon adı güncellendi
        """Nesne sorularını hazırlayan thread.""" # Yorum güncellendi
        try:
            questions = self.prepare_object_questions() # Fonksiyon adı güncellendi
            if questions:
                ...
                Clock.schedule_once(lambda dt: self._show_error_popup(
                    "Sorular hazırlanamadı. objects_images klasöründeki dosyaları kontrol edin."), 0)
    ...
    def prepare_object_questions(self): # Fonksiyon adı güncellendi
        """objects_images klasöründeki görselleri kullanarak nesne tanıma soruları hazırlar.""" # Yorum güncellendi
        questions = []
        available_objects = list(OBJECT_DATA.keys()) # Değişken adı güncellendi

        # Mevcut dosyaları kontrol et
        if not os.path.exists('objects_images'):
            print("objects_images klasörü bulunamadı!")
            return None

        # Nesne listesini karıştır - Güncellendi
        random.shuffle(available_objects)

        for correct_object in available_objects[:10]:  # Her nesneden 1 tane al, toplam 10 nesne - Güncellendi
            object_images = OBJECT_DATA[correct_object] # Değişken adı güncellendi
            selected_image = random.choice(object_images) # Değişken adı güncellendi
            image_path = f'objects_images/{selected_image}'
            ...
            # Yanlış seçenek için farklı bir nesne seç - Güncellendi
            wrong_objects = [obj for obj in available_objects if obj != correct_object] # Değişken adı güncellendi
            # Hata olasılığını azaltmak için, eğer yanlış nesne yoksa (tek bir nesne varsa listede), döngüyü kır
            if not wrong_objects:
                print(f"Uyarı: Yanlış seçenek oluşturulamıyor, sadece bir nesne var: {correct_object}")
                break
            wrong_object = random.choice(wrong_objects) # Değişken adı güncellendi

            # Seçenekleri karıştır
            options = [correct_object, wrong_object] # Değişken adı güncellendi
            random.shuffle(options)

            question = {
                "image": image_path,
                "correct_answer": correct_object, # Değişken adı güncellendi
                "options": options
            }
            questions.append(question)

        # Eğer yeterli soru oluşturulamadıysa, eksik olanları tamamla
        while len(questions) < 10 and len(available_objects) >= 2: # En az 2 nesne olmalı ki yanlış seçenek oluşturulabilsin
            # Rastgele bir nesne seç - Güncellendi
            correct_object = random.choice(available_objects)
            object_images = OBJECT_DATA[correct_object] # Değişken adı güncellendi
            selected_image = random.choice(object_images) # Değişken adı güncellendi
            image_path = f'objects_images/{selected_image}'
            ...
            # Yanlış seçenek için farklı bir nesne seç - Güncellendi
            wrong_objects = [obj for obj in available_objects if obj != correct_object] # Değişken adı güncellendi
            if not wrong_objects: # Tekrar kontrol
                break
            wrong_object = random.choice(wrong_objects) # Değişken adı güncellendi
    ...
    def load_question(self):
        if self.current_question_index < self.total_questions:
            ...
            # Resmi yükle
            try:
                self.object_image.source = question_data["image"] # Değişken adı güncellendi
            except Exception as e:
                print(f"Resim yükleme hatası: {e}")
                self.object_image.source = 'objects_images/default.jpg'  # Varsayılan resim - Güncellendi
    ...
    def finish_test(self, *args):
        ...
        report_data = {
            ...
            "konu": "Nesne Tanıma Testi", # Güncellendi
            "dogru_cevap": correct,
...
class ReportScreen(Screen):
    ...
    def _fetch_gemini_comment_thread(self, report_data):
        ...
        Bu nesne tanıma testi sonuçlarına göre öğrencinin: # Güncellendi
        - Görsel algı ve tanıma becerilerini değerlendir
        - Nesneler hakkındaki genel bilgi düzeyini yorumla # Güncellendi
