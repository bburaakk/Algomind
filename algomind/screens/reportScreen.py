...
class RaporEkrani(MDScreen):
    ...
    def _convert_markdown_to_kivymd(self, text):
        """Markdown formatını KivyMD markup'ına dönüştürür"""
        if not text:
            return text
        ...
        # *italic* -> [i]italic[/i] (tek yıldız)
        text = re.sub(r'\*(.*?)\*', r'[i]\1[/i]', text)

        # ### Başlık -> [b]Başlık[/b] (markdown başlıklar)
    ...
    def _fetch_report_analysis_thread(self, report_data):
        ...
        try:
            ...
            if result_id:
                ...
                if report_text:
                    # Markdown formatını dönüştür
                    report_text = self._convert_markdown_to_kivymd(report_text)
        ...
        try:
            ...
            payload = {
                ...
                "student_id": report_data.get('student_id',
                                              getattr(MDApp.get_running_app(), 'selected_student_id', None)),
...
""".format(success_rate)
    ...
    def _update_report_ui(self, report_data, analysis_text):
        ...
        # Eğer analysis_text'te hala markdown karakterler varsa temizle
        analysis_text = self._convert_markdown_to_kivymd(analysis_text)

        lines = analysis_text.strip().split('\n')
        analysis_text_formatted = analysis_text.strip()

        # Başlık formatlaması kontrolü
        if lines and "**" in lines[0]:
            title = lines[0].replace("**", "").strip()
            body = "\n".join(lines[2:]).strip()
            analysis_text_formatted = f"[b]{title}[/b]\n\n{body}"
