...
# Hayvan listesi ve dosya adları (images klasöründeki dosyalar)
ANIMAL_DATA = {
    "kedi": ["kedi.png"],
    "köpek": ["köpek.png"],
    "kuş": ["kuş.png"],
...
class TestSelectionScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Hayvan Tanıma Testi Butonu
        self.animal_test_button = Button(
            text='Hayvan Tanıma Testi',
            size_hint_y=None,
            height=dp(60),
            font_size='22sp',
            background_normal='',
        ...
        with self.animal_test_button.canvas.before:
            self.animal_test_button.btn_color = Color(*self.animal_test_button.background_color)
            self.animal_test_button.btn_rect = RoundedRectangle(pos=self.animal_test_button.pos,
                                                                size=self.animal_test_button.size, radius=[dp(15)] * 4)
            self.animal_test_button.bind(pos=update_button_canvas, size=update_button_canvas)
        self.animal_test_button.bind(on_press=self.start_animal_test)
        self.main_layout.add_widget(self.animal_test_button)
    ...
    def start_animal_test(self, instance):
...
class TestQuestionsScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.animal_image = Image(
            source='images/kedi1.jpg',  # Varsayılan resim
            size_hint=(None, None),
            size=(dp(150), dp(150)),
            pos_hint={'center_x': 0.5}
        )
        self.main_layout.add_widget(self.animal_image)
        ...
        self.time_elapsed = 0
        self.test_questions_data = []  # Hayvan sorularını burada saklayacağız
    ...
    def start_test(self, student_name):
        ...
        # Hayvan sorularını hazırla
        threading.Thread(target=self._prepare_animal_questions_thread).start()

    def _prepare_animal_questions_thread(self):
        """Hayvan sorularını hazırlayan thread."""
        try:
            questions = self.prepare_animal_questions()
            if questions:
    ...
    def prepare_animal_questions(self):
        """Images klasöründeki görselleri kullanarak hayvan tanıma soruları hazırlar."""
        questions = []
        available_animals = list(ANIMAL_DATA.keys())
        ...
        # Hayvan listesini karıştır
        random.shuffle(available_animals)

        for correct_animal in available_animals[:10]:  # Her hayvandan 1 tane al, toplam 10 hayvan
            animal_images = ANIMAL_DATA[correct_animal]
            selected_image = random.choice(animal_images)
            ...
            # Yanlış seçenek için farklı bir hayvan seç
            wrong_animals = [animal for animal in available_animals if animal != correct_animal]
            wrong_animal = random.choice(wrong_animals)

            # Seçenekleri karıştır
            options = [correct_animal, wrong_animal]
        ...
        while len(questions) < 10:
            # Rastgele bir hayvan seç
            correct_animal = random.choice(available_animals)
            animal_images = ANIMAL_DATA[correct_animal]
            selected_image = random.choice(animal_images)
            image_path = f'images/{selected_image}'
    ...
    def load_question(self):
        if self.current_question_index < self.total_questions:
            ...
            # Resmi yükle
            try:
                self.animal_image.source = question_data["image"]
            except Exception as e:
                print(f"Resim yükleme hatası: {e}")
                self.animal_image.source = 'images/default.jpg'  # Varsayılan resim
...
class ReportScreen(Screen):
    ...
    def _fetch_gemini_comment_thread(self, report_data):
        ...
        Bu hayvan tanıma testi sonuçlarına göre öğrencinin:
        - Görsel algı ve tanıma becerilerini değerlendir
        - Hayvanlar hakkındaki genel bilgi düzeyini yorumla
